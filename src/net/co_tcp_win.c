...
)
{
    co_socket_handle_t handle =
        co_win_tcp_socket_create(
            server->sock.local.net_addr.sa.any.ss_family);
...
)
{
    ...
    if (client->win.io_ctxs == NULL)
    {
        co_list_ctx_st list_ctx = { 0 };
        list_ctx.destroy_value = (co_item_destroy_fn)co_win_destroy_io_ctx;
        client->win.io_ctxs = co_list_create(&list_ctx);
    }
    ...
    io_connect_ctx->sock = &client->sock;

    co_list_add_tail(client->win.io_ctxs, io_connect_ctx);
...
)
{
    co_list_data_st* data =
        co_list_get_head(client->win.io_ctxs);
