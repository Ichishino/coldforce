...
)
{
    ...
    if (client->callbacks.on_timer != NULL)
    {
        client->callbacks.on_timer(thread, client);
    }
}
...
{
    if (co_socket_get_timer(&client->sock) != NULL)
    {
        return false;
    }

    co_timer_t* timer =
        co_timer_create(msec, co_tcp_client_on_timer,
            false, client);
    ...
    co_socket_set_timer(&client->sock, timer);

    return true;
}

void
co_tcp_destroy_timer(
    co_tcp_client_t* client
)
{
    if (client != NULL)
    {
        co_timer_destroy(
            co_socket_get_timer(&client->sock));
...
)
{
    ...
    return co_timer_is_running(timer);
}
